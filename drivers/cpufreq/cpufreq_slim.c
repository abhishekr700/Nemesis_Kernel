...
struct timer_list freq_mode_time;
...
{
	if (timeout > 0)
		mod_timer(&freq_mode_time, jiffies + msecs_to_jiffies(timeout));
	tbl_select[0] = 2;
	tbl_select[1] = 3;
	tbl_select[2] = 4;
	tbl_select[3] = 4;
...
static void dbs_init_freq_map_table(struct cpufreq_policy *policy)
{
	...
	init_timer(&freq_mode_time);
	freq_mode_time.function = switch_mode_timer;
	freq_mode_time.data = 0;
...
*/
int input_event_boosted_new(void)
{
	unsigned long flags;
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (dbs_tuners_ins.gboost) {
		if (g_count < 100 && graphics_boost1 < 5) {
			++g_count;
		} else if (g_count > 1) {
			--g_count;
			--g_count;
		}
		if (graphics_boost1 < 4 && g_count > 80) {
			dbs_tuners_ins.up_threshold = 60 + (graphics_boost1 * 10);
		} else {
